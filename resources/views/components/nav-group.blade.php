    ...
    <div class="nav-group" x-data="{ expanded: {{ $collapsed ? 'false' : 'true' }} }" x-cloak>
        ...
        <ul class="mt-4 space-y-2" x-show="collapsed ? false : (expanded ? true : false)">
            @foreach ($items as $item)
                <li class="2xl:pl-4">
                    <a href="{{ $item['url'] ?? '#' }}"
                        class="w-full flex items-center justify-between px-4 2xl:pl-6 py-2 text-dark-light @if ($item['active'] ?? false) bg-green-light font-medium @else hover:bg-[#828c851a] @endif rounded group">
                        {{ $item['title'] }}
                    </a>
                </li>
            @endforeach
