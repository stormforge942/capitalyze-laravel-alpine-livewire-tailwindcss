<?php $isRBE = $name === 'Revenue By Employee' || $name === 'Free Cash Flow Before WC'; ?>
...
<div x-data="{ hide: false }">
    ...
    <div class="@if ($enclosed ?? false) @else bg-white px-4 py-6 md:px-6 rounded-lg relative @endif"
        x-show="!hide" x-transition>
        ...
        <div x-data="{
            ...
            type: @if (isset($chartConfig['type'])) $wire.entangle('chartConfig.type', true) @else 'values' @endif,
            data: @js($data ?? []),
            init() {
                this.renderChart();
            ...
            renderChart() {
                ...
                window.analysisCharts['{{ $name }}'] = window.analysisPage.renderBaseChart(this.$refs.canvas, this.data, {
                    type: this.type,
                    showLabel: this.showLabel,
                    isRBE: {{ $isRBE ? 'true' : 'false' }},
                })
            },
        }" wire:key="{{ $key }}">
