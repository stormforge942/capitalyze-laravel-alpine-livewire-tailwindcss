<div class="w-100" x-data="{
    ...
    init() {
        this.$watch('settings', (val) => {
            this.$wire.updateSetting(val);
        }, { deep: true });
    }
}">
    <div class="flex flex-col">
        <span class="text-[#2575F0] font-bold border-l-4 border-blue-500 py-2 px-2">Data Settings</span>
        <span class="text-[#2575F0] block mt-2 px-3">Set default table options for the financial table</span>
    </div>

    <div class="mt-3 px-3">
        @foreach ($data_settings_columns as $key => $column)
            <div class="flex items-center justify-between py-3 border-b">
                <div>
                    <h6 class="text-md font-semibold mb-1">{{ $column['title'] }}</h6>
                    <p class="text-sm text-gray-500">{{ $column['description'] }}</p>
                </div>
                @if (($column['type'] ?? '') === 'year-range')
                    <x-select-number-range
                        unit=""
                        prefix=""
                        label="{{ $column['title'] }}"
                        longLabel="{{ $column['description'] }}"
                        x-model="settings['{{ $key }}']"></x-select-number-range>
                @elseif ($key === 'decimal')
                    <div x-data="{
                        showDropdown: false,
                        tmpValue: null,
