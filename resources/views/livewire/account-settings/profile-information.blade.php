<div class="w-100" x-data="{
    ...
    handleFileUpload(event) {
        @this.upload('profilePhoto', event.target.files[0]);
    },
}">
    <div class="flex items-center justify-between">
        <span class="text-[#2575F0] font-bold border-l-4 border-blue-500 py-2 px-2">Profile Information</span>
        @if ($isEdit)
            <button class="bg-[#52D3A2] text-black px-4 py-2 rounded font-bold" @click="confirmUpdate()">Confirm Update</button>
        @else
            <a class="underline font-bold" @click.prevent="$wire.isEdit = true;">Edit</a>
        @endif
    </div>
    ...
    <div class="mt-3 px-6">
        <div class="my-6">
            ...
            <div class="w-[120px] h-[120px] text-[50px] my-2 mr-2">
                @if ($profilePhoto)
                    <img src="{{ $profilePhoto->temporaryUrl() }}" alt="Profile Photo" class="rounded-full w-full h-full" />
                @elseif ($user->profile_photo_path)
                    <img src="{{ asset('storage/' . $user->profile_photo_path) }}" alt="Profile Photo" class="rounded-full w-full h-full">
                @else
            ...
            <div>
                <input type="file" id="file-upload" class="hidden" @change="handleFileUpload" x-bind:disabled="! $wire.isEdit">
        ...
        @if ($isEdit)
        <form x-ref="editFormRef" @submit.prevent="">
            <div class="mb-4">
