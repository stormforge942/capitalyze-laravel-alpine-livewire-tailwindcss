<div>
   <div>
        ...
        <div class="h-full px-3 py-4 overflow-y-auto bg-gray-50 dark:bg-gray-800">
            ...
            <ul class="space-y-2 font-medium">
                @if(in_array($currentRoute, ['hkex.metrics']))
                <li>
                    <!-- Settings Dropdown -->
                ...
                @foreach ($navbarItems as $navbar)
                    @if ($navbar->is_moddable && $this->showNavbar($navbar->id) && Str::startsWith($navbar->route_name, ['hkex.']))
                        <li>
                            <livewire:company-navbar-item wire:key="{{ $navbar->route_name }}-{{ $period }}"
                            href="{{ route($navbar->route_name, ['ticker' => $hkex->symbol, 'period' => $period]) }}" name="{{ $navbar->name }}"
