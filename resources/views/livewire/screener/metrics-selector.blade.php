<div x-data="{
    ...
    options: [],
    _options: [],
    dates_: @js($metricsSelector['dates']),
    tmpValue: [],
    active: null,
    ...
    get currentSelectedCount() {
        return new Set(this.tmpValue.filter(item => item.dates.length).map(item => item.metric + item.type)).size
    },
    makeOptions() {
        const options = @js($metricsSelector['options']);
    ...
    get selectedMetricsData() {
        ...
        this.tmpValue.forEach(item => {
            ...
            if (!value[key]) {
                const title = this.options[0].items[item.metric].title + ' (' + { value: 'Value', growth: '% Change YoY', cagr: 'CAGR' } [item.type] + ')'
    ...
    <div style="width: 45rem">
        <div class="px-6 pt-6 pb-2">
            <div class="flex justify-between gap-2">
                <p class="font-medium text-base">Metrics</p>
        ...
        <div class="relative mt-2 px-6">
            ...
            <div class="mt-2">
                <input type="search"
                    ...
                    placeholder="Search..." x-model.debounce.500ms="search">
            </div>
            ...
            <div class="mt-4 grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div class="overflow-y-scroll" style="max-height: 25rem">
                    <div>
                        <button type="button"
                            ...
                            <span class="h-5 w-5 grid place-content-center bg-dark text-white rounded-full text-xs"
                    ...
                    <template x-for="(option, value) in subOptions" :key="value">
                        <div x-data="{
                            ...
                            get dataTypes() {
                                let types = { value: 'Value' }
                        
                                if (option.mapping.yoy_change) {
                                    types.growth = '% Change YoY'
                                }
                            ...
                            <button type="button"
                                ...
                                @click.prevent="expand = open ? null : value; selectedMetricType = 'value'; showSelectedMetrics = false;">
                                <span class="truncate text-ellipsis" x-text="option.title"></span>

                                <svg class="shrink-0 transition-transform" width="24" height="24"
                            ...
                            <template x-if="open">
                                <div class="ml-4 space-y-4 text-sm">
                                    ...
                                    <template x-for="(label, key) in dataTypes" :key="key">
                                        <button type="button"
                                            ...
                                            @click="selectedMetricType = key; showSelectedMetrics = false;"
                                            x-text="label">
                                        </button>
                                    </template>
                                </div>
                ...
                <div class="overflow-y-scroll" style="max-height: 25rem">
                    <template x-if="open && showSelectedMetrics">
                        <div class="flex flex-col gap-y-6 px-3 h-full">
                            <template x-for="item in selectedMetricsData" :key="item.key">
                                <div>
                                    <p class="font-medium py-2" x-text="item.title"></p>

                                    <div class="space-y-2">
                                        <template x-for="(values, period) in item.values">
                                            <div type="button" class="py-2" x-data="{ open: false }">
                                                <button class="w-full flex items-center gap-x-4 justify-between"
                                                    @click="open = !open">
                                                    <span
                                                        x-text="{ annual: 'Fiscal Annual', quarter: 'Fiscal Quarterly' }[period]"></span>
                                                    <div class="inline-flex items-center gap-x-4">
                                                        <span
                                                            class="bg-green-light rounded-full text-xs font-semibold px-1.5 py-0.5"
                                                            x-text="`${values.length} SELECTION${values.length > 1 ? 'S' : ''}`"
                                                            x-cloak x-show="item.type !== 'cagr'"></span>
                                                ...
                                                <template x-if="open">
                                                    <div class="mt-4 bg-gray-light rounded p-4">
                                                        <span class="text-sm font-medium">Selected
                                                            Period</span>

                                                        <template x-if="item.type === 'cagr'">
                                                            <div
                            ...
                            <template x-if="!selectedMetricsData.length">
                                <div class="text-center m-auto">
                                    <svg class="mx-auto" width="135" height="87" viewBox="0 0 135 87"
                                        fill="none" xmlns="http://www.w3.org/2000/svg">
                                    ...
                                    <p class="mt-2 font-semibold">Metrics selected will appear here</p>
                                </div>
                            </template>
                        </div>
                    </template>
