<div class="py-3 flex-1 flex items-center justify-between border border-[#D4DDD7] rounded-lg max-w-[1230px]" x-data="{
                        ...
                        <div class="space-y-2 overflow-y-auto show-scrollbar left-scrollbar" style="max-height: 340px">
                            <template x-for="(option, idx) in options" :key="idx">
                                <div x-data="{
                                    ...
                                    <button
                                        class="text-left w-full p-4 flex items-center justify-between gap-x-4 rounded"
                                ...
                                <div class="mt-2 space-y-2 overflow-y-auto show-scrollbar" style="max-height: 310px">
                                    <template x-for="(option, value) in subOptions" :key="value">
                                        <label
                                            ...
                                            <input type="radio" name="metrics" :value="value"
                                                x-model="tmpValue" class="custom-radio border-dark focus:ring-0" />
    ...
    <div class="flex items-center children-border-right">
        <template x-if="(criteriaResultCount.financial || {}).hasOwnProperty(criteria.id)">
            <div class="pr-4">
                <span class="bg-blue font-medium text-white rounded-xl py-0.5 px-1.5 whitespace-nowrap text-sm"
                    x-text="criteriaResultCount.financial[criteria.id].toLocaleString() + ' Results'">
                </span>
            </div>
        </template>

        <button class="text-red flex-none ml-auto px-4 mr-1 text-sm font-medium hover:underline"
