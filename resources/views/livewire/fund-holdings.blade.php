        ...
        <div class="px-4 sm:px-6 lg:px-8 bg-white py-4 shadow mx-4 rounded">
            <div class="sm:flex sm:items-start flex-col">
                <div class="block mb-3">
                    <h1 class="text-base font-semibold leading-10 text-gray-900">Fund Holdings -
                        {{ Str::title($fund->name) }} - {{ Str::title($quarters[$selectedQuarter]) }}</h1>
                </div>
                <div class="block mb-3">
                    <label for="quarter-select">Quarter to view:</label>
                    <select wire:model="selectedQuarter" id="quarter-select"
            ...
            <livewire:fund-holdings-table :cik="$cik" :selectedQuarter="$selectedQuarter" />
        </div>
    </div>
</div>
@push('scripts')
    <script>
        document.addEventListener('livewire:load', function() {
            ...
            function updateUrl() {
                ...
                updateQueryParam('Quarter-to-view', selectedQuarter);
            }
