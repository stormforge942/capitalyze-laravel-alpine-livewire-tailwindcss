<div class="flex flex-col">
    <div class="flex w-full flex-row {{$selected ? 'bg-blue-100' : (count($data['children']) == 0 ? 'bg-[#52C6FF]/10' : 'bg-white')}}">
        <div
            ...
            class="cursor-default py-2 text-base w-[300px] truncate flex flex-row"
        ...
        <div class="w-full flex flex-row">
            @foreach($data['values'] as $year => $value)
                    <div class="w-[150px] flex items-center justify-center open-slide py-2 text-base  cursor-pointer hover:underline" data-value='{{$this->generateAttribute($value)}}'>
