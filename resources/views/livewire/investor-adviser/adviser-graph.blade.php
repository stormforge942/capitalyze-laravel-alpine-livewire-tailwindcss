<div x-data="{
    ...
    <div class="bg-white px-4 py-6 md:px-6 rounded-lg relative" x-show="!hide" x-transition>
        ...
        <div class="flex items-center gap-x-5">
            <b class="font-extrabold">{{ $name }}</b><br>
        </div>
        ...
        <div class="mt-3 h-[300px]" wire:loading.remove>
            <canvas id="adviser-overview-chart"></canvas>
        </div>
    </div>
</div>
...
@pushonce('scripts')
    <script>
        ...
        function initChart() {
            ...
            const canvas = document.getElementById("adviser-overview-chart");
            if (!canvas) return;
            ...
            const minEmployee = Math.min(...data.dataset2.map(i => i.y));
            ...
            chart = new Chart(ctx, {
                ...
                data: {
                    datasets: [
                        {
                            data: data.dataset1,  // Assets Under Management
                        ...
                        {
                            data: data.dataset2,  // Number of Employees
                ...
                options: {
                    ...
                    scales: {
                        y: {
                            min: minEmployee - 0.05 * minEmployee,
                            display: true,
                            ticks: {
                                crossAlign: "far",
                            }
                        ...
                        y1: {
                            ...
                            type: "linear",
                            beginAtZero: true,
                            max: Math.max(...data.dataset1.map(i => i.y)) * 7
                        }
                    }
                }
            });
