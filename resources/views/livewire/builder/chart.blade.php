<div class="chart-builder-page">
    ...
    <div class="grid place-items-center py-24" x-cloak>
    ...
    <div class="mt-6 bg-white p-6 relative rounded-lg" x-data="{
        ...
        <div class="mt-6 h-[400px]" x-data="{
            init() {
                ...
                for (let company in _data) {
                    Object.entries(_data[company]).forEach(([key, value]) => {
                        let _value = Object.entries(value).map(([x, y]) => {
                            let today = new Date().toISOString().split('T')[0];
                            
                            if(x >= '2018-01-01' && x <= today) {
                                return { x, y }
                            }
                ...
                new Chart(ctx, {
                    plugins: [window.chartJsPlugins.pointLine],
                    type: 'bar',
                    data: {
                        datasets,
                    },
