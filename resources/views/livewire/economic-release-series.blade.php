                        ...
                        <div class="mx-auto px-4 sm:px-6 lg:px-8 bg-white py-4 shadow md:mx-8 rounded md:w-12/14 2xl:w-2/3 w-full">
                            <div class="sm:flex sm:items-center">
                                <div class="sm:flex-auto text-center">
                                    <h1 class="text-xl font-bold leading-10 text-gray-900">Economic Release Series</h1>
                                </div>
                            </div>
                            ...
                            <div wire:init="getLineChart" class="p-4" wire:loading.remove>
...
document.addEventListener('livewire:load', function () {
    Livewire.on('getLineChart', function (chartData) {
        const canvas = document.getElementById('lineChart');
        canvas.style.width = '100%';

        const labels = chartData.map((data) => data.date);
        const datasets = [{label: chartData.shift().title, data: chartData.map((data) => data.value)}];
        ...
        new Chart(document.getElementById('lineChart'), dataLine);
    });
});
