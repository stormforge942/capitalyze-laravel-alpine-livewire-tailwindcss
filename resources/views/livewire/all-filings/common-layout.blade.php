    ...
    <div class="flex flex-col">
        ...
        <div class="grid grid-cols-7 gap-2 mt-4 hidden md:grid">
            ...
            <div class="col-span-5 sm:col-span-5 px-4 py-3 bg-white flex flex-wrap items-center border border-[#D4DDD7] rounded-lg">
                ...
                <div class="flex ml-2 relative justify-center mt-0 mx-0" x-data="{openFilingPop: false}" @keydown.window.escape="openFilingPop = false" @click.away="openFilingPop = false">
                    <button @click="openFilingPop = ! openFilingPop" class="border-[0.5px] border-[#D4DDD7] p-2 rounded-full flex items-center gap-x-1 bg-white hover:bg-[#E2E2E2] {{ $checkedCount ? 'bg-[#E2E2E2] border-2 border-[#9DA3A8]' : '' }} ">
                        <span class="text-sm p-x-4 font-[400] text-[#01090F]"> Select Filing Type</span>

                        @if($checkedCount)
                            <span class="bg-[#2C71F0] px-2 py-0 ml-2 leading-5 rounded-full text-[0.625rem] font-[600] text-white">{{$checkedCount}}</span>
                        @endif

                        <span :class="openFilingPop ? 'rotate-180' : ''" class="transition-transform shrink-0">
                    ...
                    <div x-show="openFilingPop" class="absolute left-0 py-2 mt-10 bg-white rounded-md shadow-xl z-50 sm:left-1/2 transform -translate-x-1/2 left-1/2 2xl:translate-x-0 2xl:left-0">
                        <x-slot name="trigger">
