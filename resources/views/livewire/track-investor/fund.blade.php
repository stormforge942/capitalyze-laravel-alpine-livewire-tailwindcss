            ...
            @foreach($investors as $item)
                <div class="box shadow-md bg-[#ffffff] p-3 m-2 border-0.5 rounded border-[#D4DDD7]">
                    <div class="flex content-center justify-between items-center">
                        ...
                        <div  wire:click.prevent="handleFavorite('{{$item->investor_name}}')" class="p-1 bg-[width:3px_3px]  hover:bg-green-500 focus:bg-green-500 {{in_array($item->investor_name, $addFavorites) ? 'bg-green-500' : '' }} shadow-sm bg-[#F5F5F5]">
                            <svg class="w-4 h-4 text-gray-300 dark:text-gray-600" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="{{in_array($item->investor_name, $addFavorites) ? '#fff' : 'currentColor' }}" viewBox="0 0 22 20">
