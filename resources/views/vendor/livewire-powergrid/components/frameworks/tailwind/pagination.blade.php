            ...
            <nav role="navigation" aria-label="Pagination Navigation" class="items-center justify-between sm:flex">
                <div class="flex justify-center mt-2 md:flex-none md:justify-end sm:mt-0">
                    ...
                    @if(!$paginator->onFirstPage())
                        <a
                            class="px-2 py-1 pt-2 m-1 text-center text-slate-600 bg-pg-primary-500 border-pg-primary-400 rounded cursor-pointer border-1 hover:bg-pg-primary-600 hover:border-pg-primary-800 dark:text-pg-primary-300"
                            wire:click="gotoPage(1)"
                        >
                            <x-livewire-powergrid::icons.chevron-double-left/>
                        </a>

                        <a class="px-2 py-1 pt-2 m-1 text-center text-slate-600 bg-pg-primary-500 border-pg-primary-400 rounded cursor-pointer border-1 hover:bg-pg-primary-600 hover:border-pg-primary-800 dark:text-pg-primary-300"
                        ...
                        @if (is_array($element))
                            ...
                            @foreach ($element as $page => $url)
                                ...
                                @if ($page == $paginator->currentPage())
                                    <span
                                        class="px-2 py-1 m-1 text-center text-blue-600 border-pg-primary-400 rounded cursor-pointer border-1 dark:bg-pg-primary-700 dark:text-white dark:text-pg-primary-300">{{ $page }}</span>
                                @elseif ($page === $paginator->currentPage() + 1 || $page === $paginator->currentPage() + 2 || $page === $paginator->currentPage() - 1 || $page === $paginator->currentPage() - 2)
                                    <a class="px-2 py-1 m-1 text-center text-slate-600 bg-pg-primary-500 border-pg-primary-400 rounded cursor-pointer border-1 hover:bg-pg-primary-600 hover:border-pg-primary-800 dark:text-pg-primary-300"
                        ...
                        @else
                            @if(method_exists($paginator,'getCursorName'))
                                <button
                                    ...
                                    class="p-2 m-1 text-center text-slate-600 bg-pg-primary-500 border-pg-primary-400 rounded cursor-pointer border-1 hover:bg-pg-primary-600 hover:border-pg-primary-800 dark:text-pg-primary-300">
                                        <x-livewire-powergrid::icons.chevron-double-left/>
                                </button>
                            @else
