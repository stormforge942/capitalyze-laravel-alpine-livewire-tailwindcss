...
class HkexMetrics extends Component
{
    public $hkex;
    public $table;
    public $metrics;
    ...
    public function getMetrics() {
        ...
        ->table('public.hkex_statements')
        ->select('json_result')
        ->where('symbol', '=', $this->hkex->symbol)
        ...
        foreach($this->segments as $segment) {
            ...
            foreach(array_keys($this->metrics) as $date) {
                if(array_key_exists($segment, $this->metrics[$date])) {
                    ...
                    $dataSend = array("symbol" => $this->hkex->symbol, "source" => 'public.hkex_statements', "date" => $date, "value" => $data);
    ...
    public function mount($hkex, $period)
    {
        $this->hkex = $hkex;
        $this->period = $period;
        $this->getNavbar();
    ...
    {
        return view('livewire.hkex-metrics');
    }
}
