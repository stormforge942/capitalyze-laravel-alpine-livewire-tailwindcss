    ...
    public function addColumns(): PowerGridEloquent
    {
        return PowerGrid::eloquent()
            ...
            ->addColumn('investor_name_formated', function (CompanyFilings $companyFilings) {
                return ('<a class="text-blue" href="' . route('company.fund', [$companyFilings->cik]) . '">' . Str::title($companyFilings->investor_name) . (!empty($companyFilings->put_call) ? ' <span class="text-sm font-bold">(' . $companyFilings->put_call . ')</span></a>' : '</a>'));
            })
